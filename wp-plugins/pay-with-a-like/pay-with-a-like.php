...
class PayWithaLike {
	...
	function __construct() {
		...
		$this->page = 'settings_page_pay-with-a-like';
	...
	*/
	...
	function wp_enqueue_scripts() {
		...
		if ( !$uploads['error'] && file_exists( $uploads['basedir'] . "/pay-with-a-like.css" ) )
			wp_enqueue_style( "pay-with-a-like", $uploads['baseurl']. "/pay-with-a-like.css", array(), $this->version );
		else if ( file_exists( $this->plugin_dir. "/css/front.css" ) )
			wp_enqueue_style( "pay-with-a-like", $this->plugin_url. "/css/front.css", array(), $this->version );
    }
	...
	function cachable() {
		...
		if ( !$this->is_cachable ) {
			...
			if ( $this->options["use_vk"]) {
				if ( $this->options["load_vk"] ) {
					wp_enqueue_script( 'vk', 'http://userapi.com/js/api/openapi.js?49' );
				}
				add_action('wp_head', array(&$this, 'init_vk_script'));
			}
...
EOFb;
	 ...
	 * VK init
	 */
	function init_vk_script ( ) { // 2983273
	//$this->options["vk_apiId"] = 2983273;

	$script = '<script type="text/javascript">
	 VK.init({apiId: '.$this->options["vk_apiId"].', onlyWidgets: true});
	</script>';
	echo $script;
		...
		if ( $method == "automatic" ) {
			...
			if ( preg_match_all( '%\[pwal( +)id="(.*?)"( +)description="(.*?)"(.*?)\](.*?)\[( *)\/pwal( *)\]%is', $content, $matches, PREG_SET_ORDER ) ){
				...
				foreach ( $matches as $m ) {
					if ( in_array( $m[2], $contents ) ) // This was already liked
	...
	function render_buttons( $id=0, $description='') {
		...
		if ( $description ) $content .= "<div class='pwal_description'>". $description . "</div>";

		$script = "";

		$content .= "<table align='center' class='pwal_buttons_box' style='margin:auto;border:none;'>";
		$content .= "<tr>";
		...
		if ( $this->options["use_facebook"] ) {
			...
			$content .= "<td align='center' style='border:none;'><div style='display:inline-block;margin:0;padding:0;width:98px;'><fb:like id='pwal_".$id."' layout='box_count' href='{$url_to_like}'></fb:like></div></td>";
			if ( !$this->script_added )
		...
		if ( $this->options["use_vk"] ) {
			$content .= "<td align='center' style='border:none;'><div style='display:inline-block;margin:0;padding:0;width:86px;'>";
			$content .= "<div style='display:inline-block;margin:0;padding:0;' id='vk_like_".$id."'></div><script type='text/javascript'>";
			$content .= "VK.Widgets.Like('vk_like_".$id."', {type: 'mini', height: 24, pageUrl:'{$url_to_like}', pageTitle: '".$post_title."'});";
			$content .= "VK.Observer.subscribe('widgets.like.liked',function(){pwal_vk_callback_".$id."();});";
			$content .= "</script></div></td>";
			if ( !$this->script_added ) $script  .= "function pwal_vk_callback_".$id."() {jQuery(document).trigger('pwal_button_action',['vk',".$id."]);}";
		}
		if ( $this->options["use_twitter"] ) {
			$content .= "<td align='center' style='border:none;'><div style='display:inline-block;margin:0;padding:0;width:82px;' class='pwal_button pwal_twitter_button' id='pwal_".$id."' ><a href='https://twitter.com/share' class='twitter-share-button' data-count='vertical' data-url='{$url_to_like}'>Tweet</a></div></td>";
			if ( !$this->script_added )
		...
		if ($this->options["buttonsrowsqty"] == 'tworows') $content .= "</tr><tr>";

		if ( $this->options["use_google"] ) {
			$content .= "<td align='center' style='border:none;'><div style='display:inline-block;margin:0;padding:0;width:54px;'><g:plusone size='tall' href='{$url_to_like}' callback='pwal_google_callback_".$id."'></g:plusone></div></td>";
		...
		if ( $this->options["use_linkedin"] ) {
			$content .= "<td align='center' style='border:none;'><div style='display:inline-block;margin:0;padding:0;width:60px;'><script type='IN/Share' data-counter='top' data-url='{$url_to_like}' data-onsuccess='pwal_linkedin_callback_".$id."'></script></div></td>";
			$script  .= "function pwal_linkedin_callback_".$id."() {
							jQuery(document).trigger('pwal_button_action',['linkedin',".$id."]);
						}
						";
		}// class='pwal_list_item_".$n."'

		if ($this->options["buttonsrowsqty"] == 'tworows') $content .= "<td style='border:none;'>&nbsp;</td>";
		...
		$content .= "</tr></table></div><div style='clear:both;'></div>";
	...
	function set_cookie( ) {
		...
		if ( !$_POST["post_id"] || !check_ajax_referer( 'ajax-nonce', 'nonce', false ) ) {
			die( json_encode( array( "error" => __('Что-то пошло не так... Перезагрузите страницу и попробуйте еще раз.', 'pwal') ) ) );
		}
	...
	function install() {
		...
		add_option( 'pwal_options', array(
										...
										'use_twitter'				=> 'true',
										'use_google'				=> 'true',
										'buttonsrowsqty'			=> 'onerow',
										'url_to_like'				=> '',
										'salt'						=> $salt,
										'sitewide'					=> '',
										'description'				=> __('Если Вы хотите увидеть содержимое всей статьи, кликните по одной из этих кнопок','pwal'),
										'random'					=> '',
										'no_visit'					=> '',
										'load_facebook'				=> 'true',
										'load_vk'					=> 'true',
	...
	function admin_init() {
		...
		if ( isset($_POST["action_pwal"]) ) {
			...
			$this->options['use_vk']				= $this->checkbox_value('use_vk');
			$this->options["vk_apiId"]				= $_POST["vk_apiId"];
			...
			$this->options["buttonsrowsqty"]		= $_POST["buttonsrowsqty"];
			...
			$this->options['load_vk']				= $this->checkbox_value('load_vk');
	 ...
	 */
	function saved( ) {
		echo '<div class="updated fade"><p>'.__("<b>Settings Saved</b>","pwal").'</p></div>';
	}
	...
	?>
		...
		<div class="postbox <?php echo $this->postbox_classes('pwal_accessibility_postbox') ?>" id="pwal_accessibility_postbox"><div class="handlediv" title="<?php echo esc_attr__('Click to toggle') ?>"><br /></div>
            ...
            <div class="inside">
				...
				<table class="form-table">
					...
					/* <![CDATA[ */
					jQuery(document).ready(function(){
						jQuery("select#authorized").change(function() {
							if ( jQuery('select#authorized').val() == "true" ) { jQuery("#level").show(); }
							else { jQuery("#level").hide(); }
						});
					});
					/* ]]> */
					</script>
            ...
            <div class="inside">
				...
				<table class="form-table">
					...
					/* <![CDATA[ */
					jQuery(document).ready(function(){
						if(!jQuery('input[name=use_facebook]').is(":checked")) {
							jQuery('input[name=load_facebook]').attr({'checked':false});
						}
						jQuery('input[name=use_facebook]').change(function() {
							if(!jQuery(this).is(":checked")) {
								jQuery('input[name=load_facebook]').attr({'checked':false});
							}
							return false;
						});
						if(jQuery('input[name=use_vk]').is(":checked")) {
							jQuery('.vkapiidbox').removeClass('pwal_hidden');
						}
						else {
							jQuery('.vkapiidbox').addClass('pwal_hidden');
							jQuery('input[name=load_vk]').attr({'checked':false});
						}
						jQuery('input[name=use_vk]').change(function() {
							if(jQuery(this).is(":checked")) {
								jQuery('.vkapiidbox').removeClass('pwal_hidden');
							}
							else {
						...
						if(!jQuery('input[name=use_twitter]').is(":checked")) {
							jQuery('input[name=load_twitter]').attr({'checked':false});
						}
						jQuery('input[name=use_twitter]').change(function() {
							if(!jQuery(this).is(":checked")) {
								jQuery('input[name=load_twitter]').attr({'checked':false});
							}
							return false;
						});
						if(!jQuery('input[name=use_google]').is(":checked")) {
							jQuery('input[name=load_google]').attr({'checked':false});
						}
						jQuery('input[name=use_google]').change(function() {
							if(!jQuery(this).is(":checked")) {
								jQuery('input[name=load_google]').attr({'checked':false});
							}
							return false;
						});
						if(!jQuery('input[name=use_linkedin]').is(":checked")) {
							jQuery('input[name=load_linkedin]').attr({'checked':false});
						}
						jQuery('input[name=use_linkedin]').change(function() {
							if(!jQuery(this).is(":checked")) {
								jQuery('input[name=load_linkedin]').attr({'checked':false});
							}
							return false;
						...
						jQuery('input[name=pwal_save_settings_btn]').click(function(){
							var vk_api_id = jQuery('input[name=vk_apiId]').val();
							if(jQuery('input[name=use_vk]').is(":checked") && !vk_api_id) {
								alert('<?php _e('Enter VK API ID!','pwal')?>');
								jQuery('input[name=vk_apiId]').focus();
								return false;
							}
							return true;
						});
					...
					<tr valign="top">
						...
						<input type="checkbox" name="use_vk" value="true" <?php if ($this->options["use_vk"]) echo "checked='checked'"?>>&nbsp<?php _e('VKontakte', 'pwal')?>&nbsp&nbsp&nbsp
					...
					<tr valign="top">
					<th scope="row"><div class="vkapiidbox pwal_hidden"><?php _e('VK API ID','pwal')?></div></th>
					<td colspan="2">
					<div class="vkapiidbox pwal_hidden"><input type="text" style="width:100px" name="vk_apiId" value="<?php echo $this->options["vk_apiId"] ?>" /> <span class="description"><?php _e('Unique ID of your Like-application in vk.com','pwal')?></span></div>
					</td>
					</tr>
					<tr valign="top">
						...
						<input type="checkbox" name="load_vk" value="true" <?php if ($this->options["load_vk"]) echo "checked='checked'"?>>&nbsp<?php _e('VKontakte', 'pwal')?>&nbsp&nbsp&nbsp
					...
					<tr valign="top">
						<th scope="row" ><?php _e('Расположение кнопок', 'pwal') ?></th>
						<td colspan="2">
						<input type="radio" name="buttonsrowsqty" value="onerow"<?php if($this->options["buttonsrowsqty"] == 'onerow' || !$this->options["buttonsrowsqty"]) echo ' checked="checked"'; ?> /> В один ряд&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="buttonsrowsqty" value="tworows"<?php checked($this->options["buttonsrowsqty"], 'tworows') ?> /> В два ряда
						</td>
					</tr>
					<tr valign="top">
					...
					/* <![CDATA[ */
					jQuery(document).ready(function(){
						jQuery("select#random").change(function() {
							if ( jQuery('select#random').val() == "true" ) { jQuery("#url_to_like").hide(); }
							else { jQuery("#url_to_like").show(); }
						});
					});
					/* ]]> */
					</script>
					...
					<p class="submit">
					<input type="submit" name="pwal_save_settings_btn" class="button-primary" value="<?php _e('Save Changes', 'pwal') ?>" />
					</p>
			</form>
			...
			<div class="postbox <?php echo $this->postbox_classes('pwal_statistics_postbox') ?>" id="pwal_statistics_postbox"><div class="handlediv" title="<?php echo esc_attr__('Click to toggle') ?>"><br /></div>
			 <?php wp_nonce_field('closedpostboxes', 'closedpostboxesnonce', false ); ?>
				...
				<div class="inside" id="pwal_stats">
					...
					if ( !$total_likes ) _e("No Data yet...", 'pwal');
					else {
						$fb = $vk = $lin = $tw = $gp = 0; // Set like counts to zero
						...
						<tr valign="top">
							<th scope="row" >
							<input type="button" id="pwal_clear_button" class="button-secondary" value="<?php _e('Очистить статистику') ?>" title="<?php _e('Нажав эту кнопку, Вы удалите всю имеющуюся статистику.') ?>" />
							</th>

							<td colspan="2">
							<form action="<?php echo admin_url('admin-ajax.php?action=pwal_export_stats'); ?>" method="post">
								<input type="submit" class="button-secondary" value="<?php _e('Експорт статистики') ?>" title="<?php _e('Нажав эту кнопку, Вы сохраните статистику в формате CSV-файла для Excel.') ?>" />
							</form>
							</td>
						</tr>
						</table>
			...
			<div class="postbox <?php echo $this->postbox_classes('pwal_instructions') ?>" id="pwal_instructions"><div class="handlediv" title="<?php echo esc_attr__('Click to toggle') ?>"><br /></div>
				...
				<div class="inside">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;echo wpmudev_pwal( custom_description(), true );<br />
					else<br />
					...
					printf(__('If you want to apply your own styles copy and rename the default css file <b>%s</b> as <b>%s</b> and edit this latter file. Then, your edited styles will not be affected from plugin updates.', 'pwal'), $default_css, $custom_css);
					?>
					<br />
				</div>
			</div>
		...
		/* <![CDATA[ */
		jQuery(document).ready(function() {
			jQuery('#pwal_clear_button').click(function() {
				if ( !confirm('<?php _e("Are you sure to clear statistics?","pwal") ?>') ) {return false;}
					...
					jQuery.post(ajaxurl, data, function(response) {
						if ( response && response.error )
							...
							jQuery("#pwal_stats").html('<?php _e("Statistics cleared...","pwal") ?>');
						}
					},'json');
				}
			});
